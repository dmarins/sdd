...
// PT-BR fork: accept the canonical "Use quando …" plus the equivalent
// "Use ao/antes/durante/depois …" phrasings in use today.
// Reject negated forms ("Não use quando …", "Nunca use ao …") — those
// describe exclusions, not trigger conditions.
const DESCRIPTION_TRIGGER        = /\buse (esta skill )?quando\b|\buse (ao|antes|durante|depois)\b/i;
const DESCRIPTION_TRIGGER_NEGATE = /\b(não|nao|nunca) use (esta skill )?(quando|ao|antes|durante|depois)\b/i;
...
const REQUIRED_SECTIONS = [
  ['## Visão Geral'],
  ['## Quando Usar'],
  ['## Racionalizações Comuns', '## Justificativas Comuns'],
  ['## Sinais de Alerta'],
  ['## Verificação'],
];
...
const SECTION_EXEMPT_SKILLS = {
  ...
  'go-aws-serverless-development':      'Local fork skill — execution profile organized as layered workflow; has no Rationalizations/Red-Flags anatomy.',
  'go-runtime-and-dependency-upgrades': 'Local fork skill — upgrade runbook organized by phases; has no Rationalizations/Red-Flags anatomy.',
};
