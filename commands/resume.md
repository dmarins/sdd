...
4. Destaque lições `OPEN` e lições `PROMOTED` recentemente que sejam relevantes para a task ativa
5. Atualize `/docs/handoff.md` com o contexto reidratado: task ativa, arquivos já tocados, verificações já concluídas, blockers, lições relevantes e próximo passo
6. Se `/docs/handoff.md` estiver ausente ou desatualizado, reconstrua um estado mínimo confiável antes de voltar a implementar
7. Retome a execução a partir do menor incremento verificável, em vez de assumir que o trabalho parcial estava concluído
